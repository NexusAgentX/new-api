...
func RelaySwapFace(c *gin.Context, info *relaycommon.RelayInfo) *dto.MidjourneyResponse {
	...
	if !priceData.FreeModel {
		...
		defer func() {
			if !billingCommitted && info.Billing != nil {
	...
	defer func() {
		if mjResp.StatusCode == 200 && mjResp.Response.Code == 1 {
			...
			model.RecordConsumeLog(c, info.UserId, model.RecordConsumeLogParams{
				...
				QuotaAfterGroupUnrounded: priceData.QuotaAfterGroupUnrounded,
				HasQuotaCalculation:      priceData.Quota == 0 || priceData.QuotaBeforeGroup > 0 || priceData.QuotaAfterGroupUnrounded > 0,
				Content:                  logContent,
...
func RelayMidjourneySubmit(c *gin.Context, relayInfo *relaycommon.RelayInfo) *dto.MidjourneyResponse {
	...
	if consumeQuota && !priceData.FreeModel {
		...
		defer func() {
			if !billingCommitted && relayInfo.Billing != nil {
