...
func LogTaskConsumption(c *gin.Context, info *relaycommon.RelayInfo) model.ChannelFinanceValues {
	...
	channelFinance := model.RecordConsumeLog(c, info.UserId, model.RecordConsumeLogParams{
		...
		QuotaAfterGroupUnrounded: info.PriceData.QuotaAfterGroupUnrounded,
		HasQuotaCalculation:      info.PriceData.Quota == 0 || info.PriceData.QuotaBeforeGroup > 0 || info.PriceData.QuotaAfterGroupUnrounded > 0,
		Content:                  logContent,
...
func RecalculateTaskQuotaByTokens(ctx context.Context, task *model.Task, totalTokens int) {
	...
	quotaCost := float64(totalTokens) * modelRatio * finalGroupRatio * otherMultiplier
	actualQuota, clamp := common.QuotaFromFloatChecked(quotaCost)
	actualQuota = common.ApplyMinimumBillableQuota(
		actualQuota,
		quotaCost > 0,
		ratio_setting.IsZeroQuotaAllowed(group),
	)
